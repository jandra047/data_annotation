...
class NewProjectForm(FlaskForm):
    name = StringField('Project name', validators=[DataRequired()], render_kw={
                       'placeholder': 'Project name'})
    images = MultipleFileField('Photos', validators=[
                               DataRequired(), ImageFileRequired()])

    def validate_name(self, name):
        project = current_user.projects.join(User2Project).filter(
            User2Project.role == 'admin', Project.name == name.data).one_or_none()
