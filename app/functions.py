...
def create_segments(image_path, algorithm='Slic', n_segments=200, compactness=10):
    ...
    if algorithm == 'Slic':
        segments = slic(image, n_segments=n_segments,
                        compactness=compactness, sigma=0, multichannel=True).ravel()
        ...
        segments = felzenszwalb(
            image, scale=100, sigma=0.5, min_size=50).ravel()
        segments = np.repeat(segments, 4).tolist()
    else:
        segments = quickshift(image, kernel_size=3,
                              max_dist=6, ratio=0.5).ravel()
        segments = np.repeat(segments, 4).tolist()
    return segments
