...
done_images = db.Table('done_images', db.Model.metadata,
                       db.Column('user2project_id', db.Integer, db.ForeignKey(
                           'user2project.id', ondelete="CASCADE")),
...
class User(db.Model, UserMixin):
    ...
    projects = db.relationship(
        'Project', secondary='user2project', lazy='dynamic')
...
class Project(db.Model, UserMixin):
    ...
    def __init__(self, **kwargs):
        ...
        self.home_path = os.path.join(
            app.config['PROJECTS_DIR'], f'{current_user.username}_{self.name}')
        pathlib.Path(os.path.join(self.home_path, 'images')
...
class Image(db.Model, UserMixin):
    ...
    done = db.relationship('Mask', backref='image',
                           lazy='dynamic', cascade='all, delete-orphan')
...
class User2Project(db.Model, UserMixin):
    ...
    __table_args__ = (UniqueConstraint(
        'user_id', 'project_id', name='user_project_uc'),)
    id = db.Column(db.Integer, primary_key=True)
    ...
    user = db.relationship('User', backref=db.backref(
        "user2project", cascade="all, delete-orphan", lazy='dynamic'))
    project = db.relationship('Project', backref=db.backref(
        "user2project", cascade="all, delete-orphan", lazy='dynamic'))
    done_images = db.relationship(
        "Image", secondary=done_images, backref="user2project")

    def __init__(self, **kwargs):
        super(User2Project, self).__init__(**kwargs)
        self.home_path = os.path.join(
            self.project.home_path, 'users', self.user.username)
...
class Mask(db.Model, UserMixin):
    __tablename__ = 'masks'
    __table_args__ = (UniqueConstraint(
        'image_id', 'user2project_id', name='user_project_image_uc'),)
    id = db.Column(db.Integer, primary_key=True)
    image_id = db.Column(db.Integer, db.ForeignKey(
        'images.id', ondelete='CASCADE'))
    user2project_id = db.Column(db.Integer, db.ForeignKey(
        'user2project.id', ondelete='CASCADE'))
